...
export function activate(context: vscode.ExtensionContext) {
    ...
    const compileCommand = vscode.commands.registerCommand('jvav.compile', () => {
        ...
        document.save().then(() => {
            ...
            const compilerPath = config.get<string>('compilerPath', 'jvavc');
            const optimizationLevel = config.get<string>('optimizationLevel', 'O0');
            const targetPlatform = config.get<string>('targetPlatform', 'wasm');
    ...
    const startReplCommand = vscode.commands.registerCommand('jvav.startRepl', () => {
        const config = vscode.workspace.getConfiguration('jvav');
        const terminalPath = config.get<string>('terminalPath', 'jvav_terminal');
        const targetPlatform = config.get<string>('targetPlatform', 'wasm');
